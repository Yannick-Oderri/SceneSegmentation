...
int GLViewport::CreateGeometries() {
    ...
    camera_ = std::unique_ptr<GLCamera>(new GLCamera(glm::vec3(18.0075f, -3.0404f, 32.3039f)));
...
int GLViewport::RenderFrame(cv::Mat camera_image,cv::Mat point_cloud) {
    ...
    if (glfwGetKey(window_, GLFW_KEY_ESCAPE ) != GLFW_PRESS)
    {
        ...
        for (int i = 0; i < point_cloud.rows; i++)
        {
            ...
            {
                cv::Vec3f pVal = point_cloud.at<cv::Vec3f>(i, j);

                // if (count < 10000 && pVal[0] != std::numeric_limits<float>::quiet_NaN())
                {
                    count++;
                    cv::Vec3f p1(pVal[0], -pVal[1], -pVal[2]);
                    {
...
//        }
        ...
        glm::mat4 projection = glm::perspective(glm::radians(camera_->getZoom()), (float)window_width_ / (float)window_height_, 0.1f, 10000.0f);
        ...
        glBindBuffer(GL_ARRAY_BUFFER, point_cloud_vbo_);
        glBufferData(GL_ARRAY_BUFFER, xyz_color_points.size()*sizeof(cv::Vec3f), &xyz_color_points.front(), GL_STATIC_DRAW);
        glPointSize(1.5);
