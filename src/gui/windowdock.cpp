...
#include "gui/windowsizehelpers.h"
...
WindowDock::WindowDock(Edge edge)
{
    m_edge = edge;
    static int dockId(0);
    m_windowName = "Dock_";
...
void WindowDock::PushDockControl(std::unique_ptr<DockControl> &&dockControl)
{
    m_dockControls.emplace(std::move(dockControl));
    m_userHasResized = false;
}

void WindowDock::SetRegion(ImVec2 position, ImVec2 size)
{
...
void WindowDock::Show(ImVec2 regionPosition, ImVec2 regionSize)
{
    ...
    if (ImGui::Begin(m_windowName.c_str(), nullptr, DockWindowFlags))
    {
        ...
        if (m_dockControls.top()->Show() == DockControlStatus::ShouldClose)
        {
            m_dockControls.pop();
            m_userHasResized = false;
        }
...
ImVec2 WindowDock::GetSize()
{
    return m_size;
}

void WindowDock::SetSize(ImVec2 size)
{
